...
# Several upstreams (NCBI E-utilities, Unpaywall, etc.) throttle or 403 clients
# that do not identify themselves. Send a descriptive UA on every request.
HTTP_USER_AGENT = os.getenv(
    "HTTP_USER_AGENT",
    "TargetvalGateway/2025.10 (+https://targetval-gateway.onrender.com)",
)
DEFAULT_HEADERS = {"User-Agent": HTTP_USER_AGENT, "Accept": "application/json"}
...
# GraphQL templates for the Open Targets Platform v4 API.
# The legacy v3/Genetics schema (gene(ensemblId:){geneticAssociations,...}) was
# retired; the unified Platform exposes target-disease evidence via
# target(ensemblId:){associatedDiseases{...datatypeScores}}. The genetic
# evidence for these modules lives under the "genetic_association" datatype
# score. All four genetics modules share this stable, schema-valid query and
# filter the relevant datatype/disease downstream.
_OT_TARGET_ASSOCIATIONS = """
query TargetAssociations($ensg: String!) {
...
GQL_TEMPLATES = {
    "genetics-l2g": _OT_TARGET_ASSOCIATIONS,
    "genetics-coloc": _OT_TARGET_ASSOCIATIONS,
    "genetics-pqtl": _OT_TARGET_ASSOCIATIONS,
    "genetics-mqtl-coloc": _OT_TARGET_ASSOCIATIONS,
}
...
# ============================================================================
# Domain grouping (consumed by app.main for /domains discovery)
# Modules are grouped by key prefix into the six evidence domains.
# ============================================================================
DOMAINS_META: Dict[int, Dict[str, str]] = {
    1: {"name": "Genetics & Causal Variants"},
    2: {"name": "Expression & Localization"},
    3: {"name": "Association & Omics"},
    4: {"name": "Biology, Mechanism & Perturbation"},
    5: {"name": "Clinical, Safety & Tractability"},
    6: {"name": "Competitive & IP"},
}

_DOMAIN_PREFIXES: Dict[int, Tuple[str, ...]] = {
    1: ("genetics-",),
    2: ("expr-", "sc-"),
    3: ("assoc-",),
    4: ("biology-", "function-", "mech-", "perturb-"),
    5: ("clin-", "tract-", "immuno"),
    6: ("comp-",),
}

def _build_domain_modules() -> Dict[int, List[str]]:
    out: Dict[int, List[str]] = {i: [] for i in DOMAINS_META}
    for key in sorted(MODULES.keys()):
        for domain_id, prefixes in _DOMAIN_PREFIXES.items():
            if key.startswith(prefixes):
                out[domain_id].append(key)
                break
    return out

DOMAIN_MODULES: Dict[int, List[str]] = _build_domain_modules()
...
async def run_module(module_key: str, ctx: Dict[str, Any]) -> EvidenceEnvelope:
    ...
    def _finalize(data: Any, source: str, *, fallback: bool = False) -> EvidenceEnvelope:
        env.records = [{"raw": data}]
        env.edges = [Edge(
            kind="module_result",
            src=module_key,
            dst=env.context.gene_id,
            meta={"module": module_key, "stage": mod.qtl_stage, "fallback": fallback},
        )]
    ...
    try:
        if mod.family == "graphql":
            ...
            variables = {"ensg": ctx.get("ensg") or ""}
        ...
        return _finalize(data, mod.primary["url"])
        ...
        for fb in mod.fallbacks:
            try:
                ...
                prov.warnings.append(f"primary failed ({e.detail}); used fallback {fb['url']}")
                return _finalize(data, fb["url"], fallback=True)
            except Exception:
                continue
        # No source succeeded: return a structured ERROR envelope rather than
        # raising, so callers get a 200 with a clear status instead of a 500.
        if e.status_code == 429:
            env.status = EvidenceStatus.RATE_LIMITED
        elif e.status_code == 503:
            env.status = EvidenceStatus.CIRCUIT_OPEN
        else:
            env.status = EvidenceStatus.UPSTREAM_ERROR
        env.error = str(e.detail)
        prov.warnings.append(str(e.detail))
        env.provenance = prov
        return env
