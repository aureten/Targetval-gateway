...
# =============================================================================
# PATCH BLOCK (router-level, no __future__ imports) — Live, keyless fixes
# =============================================================================
# This block overrides ONLY the failing endpoints by replacing them on the
# module-level APIRouter `router`. It does not touch working modules.
# It also defines a safe EXTERNAL_URLS default if missing.
try:
...
_TVFIX_MAX_TRIES = 4
_TVFIX_BACKOFF_BASE = 0.6
_TVFIX_PER_HOST = int(os.getenv("PER_HOST_LIMIT", "4"))
_TVFIX_ALLOW = {
    "api.platform.opentargets.org", "rest.ensembl.org", "eutils.ncbi.nlm.nih.gov",
    "reactome.org", "analysis.reactome.org", "www.encodeproject.org",
    "data.4dnucleome.org", "www.ebi.ac.uk", "www.metabolomicsworkbench.org",
    "api.opengwas.io", "gwas-api.mrcieu.ac.uk",
}
...
async def _tvfix_fetch_json(url: str, *, method: str = "GET",
                            params: Optional[Dict[str, Any]] = None,
                            headers: Optional[Dict[str, str]] = None,
                            json_body: Optional[Dict[str, Any]] = None,
                            data: Optional[str] = None,
                            allow_status: Tuple[int, ...] = (200, 204)) -> Tuple[str, int, Optional[Dict[str, Any]]]:
                ...
                try:
                    ...
                    if resp.status_code in allow_status:
                        ...
                        try:
                            ...
                            return "NO_DATA", resp.status_code, {"note":"Non-JSON", "text": resp.text[:1000]}
                    if resp.status_code == 401 and ("opengwas" in nl or "gwas-api" in nl):
                        return "UPSTREAM_AUTH_REQUIRED", resp.status_code, {"error":"OpenGWAS requires JWT"}
                    if 400 <= resp.status_code < 600 and tries < _TVFIX_MAX_TRIES:
                        await asyncio.sleep(_TVFIX_BACKOFF_BASE * (2 ** (tries - 1))); continue
...
async def _tvfix_symbol_to_ensg(symbol: str) -> Optional[str]:
    ...
    s, c, j = await _tvfix_fetch_json(url, params={"content-type": "application/json"})
    if s != "OK" or not isinstance(j, list): return None
    for row in j:
        if row.get("type") == "gene" and str(row.get("id","")).startswith("ENSG"): return row["id"]
    return None

async def _tvfix_efo_lookup(term: str) -> Optional[str]:
    url = "https://www.ebi.ac.uk/ols4/api/search"
    s, c, j = await _tvfix_fetch_json(url, params={"q": term, "ontology":"efo", "rows":1, "queryFields":"label,synonym"})
    try:
...
async def _tvfix_ot_graphql(query: str, variables: Dict[str, Any]):
...
# -- helper to replace routes on APIRouter
def _tvfix_router_remove(path: str):
    new_routes = []
...
def _tvfix_router_replace(path: str, handler):
    _tvfix_router_remove(path)
    router.add_api_route(path, handler, methods=["GET"])

# ---- endpoint implementations (same as prior patch, registered on router) ----
async def _tvfix_ep_genetics_coloc(symbol: str = Query(...), condition: Optional[str] = Query(None)):
    module = "genetics-coloc"
    ensg = await _tvfix_symbol_to_ensg(symbol)
    if not ensg: return _tvfix_no_data(module, {"resolver":"ensembl xrefs"}, f"Cannot resolve symbol={symbol}")
    efo = await _tvfix_efo_lookup(condition) if condition else None
    q = """
    query Q($ensg:String!,$page:Pagination){ target(ensemblId:$ensg){ credibleSets(page:$page){ rows{
      studyLocusId colocalisation(page:{index:0,size:50}){ rows{
        h4 clpp rightStudyType otherStudyLocus{ studyId study{ traitFromSource traitFromSourceMappedIds } }
      } } } } } }
    """
    s,c,j = await _tvfix_ot_graphql(q, {"ensg": ensg, "page":{"index":0,"size":40}})
    if s != "OK": return _tvfix_upstream_error(module, {"host":"api.platform.opentargets.org"}, c, json.dumps(j)[:500])
    rows = (((j or {}).get("data") or {}).get("target") or {}).get("credibleSets", {}).get("rows", []) or []
    out=[]
    for r in rows:
        for c2 in (r.get("colocalisation") or {}).get("rows", []) or []:
            other = c2.get("otherStudyLocus", {}) or {}
            study = other.get("study", {}) or {}
            efo_ids = study.get("traitFromSourceMappedIds") or []
            if efo and not any(x==efo or (efo and efo in (x or "")) for x in efo_ids): continue
            out.append({
                "study_locus_id": r.get("studyLocusId"), "h4": c2.get("h4"), "clpp": c2.get("clpp"),
                "right_study_type": c2.get("rightStudyType"), "other_study_id": other.get("studyId"),
                "other_trait": study.get("traitFromSource"), "other_trait_efo": efo_ids,
            })
    if not out: return _tvfix_no_data(module, {"host":"api.platform.opentargets.org","target":ensg, "efo": efo}, "No colocalisations")
    return _tvfix_ok({"records": out, "provenance":{"host":"api.platform.opentargets.org"}}, module)

async def _tvfix_ep_genetics_l2g(symbol: str = Query(...)):
    module = "genetics-l2g"
    ensg = await _tvfix_symbol_to_ensg(symbol)
    if not ensg: return _tvfix_no_data(module, {"resolver":"ensembl xrefs"}, f"Cannot resolve symbol={symbol}")
    q = """
    query Q($ensg:String!){ target(ensemblId:$ensg){ credibleSets(page:{index:0,size:50}){ rows{
      studyLocusId l2GPredictions(page:{index:0,size:25}){ rows{ id score } }
    } } } }
    """
    s,c,j = await _tvfix_ot_graphql(q, {"ensg": ensg})
    if s != "OK": return _tvfix_upstream_error(module, {"host":"api.platform.opentargets.org"}, c, json.dumps(j)[:500])
    rows = (((j or {}).get("data") or {}).get("target") or {}).get("credibleSets", {}).get("rows", []) or []
    scores=[]
    for r in rows:
        for p in (r.get("l2GPredictions") or {}).get("rows", []) or []:
            if p.get("id") == ensg: scores.append({"study_locus_id": r.get("studyLocusId"), "l2g": p.get("score")})
    if not scores: return _tvfix_no_data(module, {"host":"api.platform.opentargets.org","target":ensg}, "No L2G predictions")
    return _tvfix_ok({"target": ensg, "predictions": scores, "provenance":{"host":"api.platform.opentargets.org"}}, module)

async def _tvfix_ep_genetics_consortia_summary(condition: str = Query(...)):
    module = "genetics-consortia-summary"
    efo = await _tvfix_efo_lookup(condition)
    if not efo: return _tvfix_no_data(module, {"resolver":"EFO OLS4"}, f"Cannot map condition={condition}")
    q = """
    query Q($efo:String!){ studies(diseaseIds:[$efo], enableIndirect:true, page:{index:0,size:50}){ rows{
      projectId studyType cohorts
    } } }
    """
    s,c,j = await _tvfix_ot_graphql(q, {"efo": efo})
    if s != "OK": return _tvfix_upstream_error(module, {"host":"api.platform.opentargets.org"}, c, json.dumps(j)[:500])
    rows = (((j or {}).get("data") or {}).get("studies") or {}).get("rows", []) or []
    if not rows: return _tvfix_no_data(module, {"host":"api.platform.opentargets.org","efo":efo}, "No studies found")
    by={}
    for srow in rows:
        proj = srow.get("projectId") or "unknown"
        entry = by.setdefault(proj, {"projectId": proj, "n":0, "types":set(), "cohorts":set()})
        entry["n"] += 1
        if srow.get("studyType"): entry["types"].add(srow["studyType"])
        for ccc in srow.get("cohorts") or []: entry["cohorts"].add(ccc)
...
async def _tvfix_ep_genetics_mr(exposure: Optional[str] = Query(None), outcome: Optional[str] = Query(None)):
    module="genetics-mr"
    s,c,j = await _tvfix_fetch_json("https://api.opengwas.io/api/status")
    if s != "OK": return _tvfix_upstream_error(module, {"host":"api.opengwas.io"}, c, json.dumps(j)[:500])
    return _tvfix_auth_required(module, {"host":"api.opengwas.io"}, "OpenGWAS MR endpoints require JWT; router is keyless per spec.")

async def _tvfix_ep_genetics_rare(symbol: str = Query(...)):
    ...
    for uid in (docs.get("uids", []) or [])[:20]:
        rec=docs.get(uid, {}); out.append({"uid":uid,"title":rec.get("title"),"clinical_significance":(rec.get("clinical_significance", {}) or {}).get("description"),"rcv_accession":rec.get("accessionversion")})
    return _tvfix_ok({"records": out, "provenance":{"host":"eutils.ncbi.nlm.nih.gov"}}, module)

async def _tvfix_ep_assoc_metabolomics(symbol: str = Query(...)):
    ...
    if not hits: return _tvfix_no_data(module, {"hosts":["ebi.ac.uk","metabolomicsworkbench.org"]}, "No gene mentions in metabolomics metadata")
    return _tvfix_ok({"records": hits, "provenance":{"hosts":["ebi.ac.uk","metabolomicsworkbench.org"]}}, module)

async def _tvfix_ep_genetics_3d_maps(symbol: str = Query(...)):
...
async def _tvfix_ep_genetics_annotation(symbol: str = Query(...)):
...
async def _tvfix_ep_genetics_caqtl_lite(symbol: str = Query(...), tissue: Optional[str] = Query(None)):
    module="genetics-caqtl-lite"
    # Composite loopback into our own endpoints (if present)
    try:
        async with httpx.AsyncClient(timeout=_TVFIX_DEFAULT_TIMEOUT) as client:
            ...
            if r1.status_code==200:
                d1=r1.json(); 
                if isinstance(d1, dict) and d1.get("status")=="OK": data.extend(d1.get("records", []))
            if r2.status_code==200:
                d2=r2.json();
                if isinstance(d2, dict) and d2.get("status")=="OK": data.extend(d2.get("records", []))
            if not data: return _tvfix_no_data(module, {"loopback": True}, "No regulatory/chromatin contacts available")
            return _tvfix_ok({"records": data[:50], "provenance":{"composed":["genetics/regulatory","genetics/chromatin-contacts"]}}, module)
    except Exception as e:
        return _tvfix_no_data(module, {"loopback": True}, f"Composite failed: {e}")

async def _tvfix_ep_genetics_regulatory(symbol: str = Query(...)):
...
async def _tvfix_ep_mech_pathways(symbol: str = Query(...), condition: Optional[str] = Query(None)):
...
# ---- register replacements on APIRouter ----
_tvfix_router_replace("/genetics/coloc", _tvfix_ep_genetics_coloc)
_tvfix_router_replace("/genetics/consortia-summary", _tvfix_ep_genetics_consortia_summary)
_tvfix_router_replace("/genetics/l2g", _tvfix_ep_genetics_l2g)
_tvfix_router_replace("/genetics/mr", _tvfix_ep_genetics_mr)
_tvfix_router_replace("/genetics/rare", _tvfix_ep_genetics_rare)
_tvfix_router_replace("/genetics/3d-maps", _tvfix_ep_genetics_3d_maps)
_tvfix_router_replace("/genetics/annotation", _tvfix_ep_genetics_annotation)
_tvfix_router_replace("/genetics/caqtl-lite", _tvfix_ep_genetics_caqtl_lite)
_tvfix_router_replace("/genetics/regulatory", _tvfix_ep_genetics_regulatory)
_tvfix_router_replace("/mech/pathways", _tvfix_ep_mech_pathways)

# dashed legacy aliases on router (top-level)
router.add_api_route("/genetics-coloc", _tvfix_ep_genetics_coloc, methods=["GET"])
router.add_api_route("/genetics-consortia-summary", _tvfix_ep_genetics_consortia_summary, methods=["GET"])
router.add_api_route("/genetics-l2g", _tvfix_ep_genetics_l2g, methods=["GET"])
router.add_api_route("/genetics-mr", _tvfix_ep_genetics_mr, methods=["GET"])
router.add_api_route("/genetics-rare", _tvfix_ep_genetics_rare, methods=["GET"])
router.add_api_route("/genetics-3d-maps", _tvfix_ep_genetics_3d_maps, methods=["GET"])
router.add_api_route("/genetics-annotation", _tvfix_ep_genetics_annotation, methods=["GET"])
router.add_api_route("/genetics-caqtl-lite", _tvfix_ep_genetics_caqtl_lite, methods=["GET"])
router.add_api_route("/genetics-regulatory", _tvfix_ep_genetics_regulatory, methods=["GET"])
router.add_api_route("/mech-pathways", _tvfix_ep_mech_pathways, methods=["GET"])

# health alias (router-level)
router.add_api_route("/healthz", lambda: {"ok": True, "version": "2025.10", "patch": True, "router_level": True}, methods=["GET"])
# =============================================================================



async def _tvfix_ep_genetics_nmd_inference(symbol: str = Query(...)):
    module = "genetics-nmd-inference"
    return _tvfix_no_data(module, {"note":"Composite requires sQTL + coding context; excluded in keyless patch"}, "Not implemented in keyless patch")

# Register nmd-inference
_tvfix_router_replace("/genetics/nmd-inference", _tvfix_ep_genetics_nmd_inference)
router.add_api_route("/genetics-nmd-inference", _tvfix_ep_genetics_nmd_inference, methods=["GET"])
