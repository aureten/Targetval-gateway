...
from app.hybrid import hybridize, get_source_mode, SourceMode
...
# -----------------------------------------------------------------------------
async def __core__genetics_l2g(gene: str,
...
hybrid_genetics_l2g = hybridize('genetics_l2g', __core__genetics_l2g, snapshot_fn=None)

@router.get("/genetics/l2g", response_model=Evidence)
async def genetics_l2g(gene: str,
    efo_id: str = Query(..., alias="efo"),
    limit: int = Query(50, ge=1, le=200),
    ensembl: Optional[str] = Query(None, description="Optional Ensembl gene ID override"),
) -> Evidence:
    ...
    try:
        ...
        rows = []
        # v2 returns a HAL-style response with _embedded perhaps; handle both list/dict
        if isinstance(res, dict, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_genetics_l2g(gene=gene, efo_id=efo_id, limit=limit, ensembl=ensembl, ) -> Evidence=) -> Evidence, field_name=field_name, field_name=field_name, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )

        base=)

        base, "geneName"="geneName", "size"="size", tries=tries, dict=dict, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
        if isinstance(payload, dict) and 'status' in payload and 'data' in payload:
            return Evidence(**payload)
        if hasattr(payload, 'model_dump'):
            return payload  # already a Pydantic model
    except Exception:
        pass
    return payload

async def __core__genetics_rare(gene: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
    validate_symbol(gene, field_name="gene")
    clinvar_url = (
...
hybrid_genetics_rare = hybridize('genetics_rare', __core__genetics_rare, snapshot_fn=None)
...
async def genetics_mendelian(
    ...
    try:
        ...
        items = []
        # The JSON structure includes 'data' or array; be defensive.
        if isinstance(cg, dict, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_genetics_rare(gene=gene, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, {}).get("idlist"={}).get("idlist", [])
            return Evidence(
                status=[])
            return Evidence(
                status, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
        except Exception=)
        except Exception, "variables"="variables", }
        try=}
        try, query=query, tries=tries, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
        except Exception=)
        except Exception, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )


    @router.get("/genetics/mendelian"=)


    @router.get("/genetics/mendelian", response_model=response_model, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__genetics_mr(gene: str,
    efo_id: str = Query(..., alias="efo"),
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
    """
...
hybrid_genetics_mr = hybridize('genetics_mr', __core__genetics_mr, snapshot_fn=None)

@router.get("/genetics/mr", response_model=Evidence)
async def genetics_mr(gene: str,
    efo_id: str = Query(..., alias="efo"),
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
    """
    ...
    Strategy (pragmatic, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_genetics_mr(gene=gene, efo_id=efo_id, limit=limit, ) -> Evidence=) -> Evidence, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__genetics_lncrna(gene: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
    validate_symbol(gene, field_name="gene")
...
hybrid_genetics_lncrna = hybridize('genetics_lncrna', __core__genetics_lncrna, snapshot_fn=None)

@router.get("/genetics/lncrna", response_model=Evidence)
async def genetics_lncrna(gene: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
    validate_symbol(gene, field_name="gene")
    url = f"https://rnacentral.org/api/v1/rna?q={urllib.parse.quote(gene)}&page_size={limit}"
...
async def genetics_sqtl(
    ...
    try:
        ...
        info = await _get_json(lookup_url, tries=1)
        sym_cites.append(lookup_url)
        if isinstance(info, dict) and info.get("version", mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_genetics_lncrna(gene=gene, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, []) if isinstance(js=[]) if isinstance(js, dict) else []
            return Evidence(
                status=dict) else []
            return Evidence(
                status, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
    ...
    @router.get("/genetics/mirna"=)


    @router.get("/genetics/mirna", response_model=response_model, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, "idOpt"="idOpt", "myList"="myList", "selSource"="selSource", payload=payload, tries=tries, []) if isinstance(r=[]) if isinstance(r, dict) else (r or [])
            if rows=dict) else (r or [])
            if rows, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
        except Exception=)
        except Exception, tries=tries, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
            header=)
            header, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
        except Exception as e=)
        except Exception as e, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )


    @router.get("/genetics/sqtl"=)


    @router.get("/genetics/sqtl", response_model=response_model, limit=limit, ensembl=ensembl, ) -> Evidence=) -> Evidence, ENSG00000122971.9).
        Docs=ENSG00000122971.9).
        Docs, field_name=field_name, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )

        # Fetch Ensembl version to construct versioned GENCODE id
        gencode_id=)

        # Fetch Ensembl version to construct versioned GENCODE id
        gencode_id, tries=tries, dict) and info.get("version"=dict) and info.get("version", mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__genetics_epigenetics(gene: str,
...
hybrid_genetics_epigenetics = hybridize('genetics_epigenetics', __core__genetics_epigenetics, snapshot_fn=None)

@router.get("/genetics/epigenetics", response_model=Evidence)
async def genetics_epigenetics(gene: str,
    flank_kb: int = Query(50, ge=0, le=1000),
) -> Evidence:
    """
    Functional epigenomics support via **Roadmap Epigenomics** and **BLUEPRINT** resources.

    Implementation (lightweight but robust, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_genetics_epigenetics(gene=gene, flank_kb=flank_kb, ) -> Evidence=) -> Evidence, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__assoc_bulk_rna(condition: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
...
hybrid_assoc_bulk_rna = hybridize('assoc_bulk_rna', __core__assoc_bulk_rna, snapshot_fn=None)

@router.get("/assoc/bulk-rna", response_model=Evidence)
async def assoc_bulk_rna(condition: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
    validate_condition(condition, field_name="condition")
    url = f"https://gtexportal.org/api/v2/association/genesByTissue?tissueSiteDetail={urllib.parse.quote(condition)}"
...
async def assoc_bulk_prot(
    ...
    try:
        js = await _get_json(url, tries=1)
        records: List[Any] = []
        if isinstance(js, dict, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_assoc_bulk_rna(condition=condition, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, []) if isinstance(js=[]) if isinstance(js, dict) else []
            return Evidence(
                status=dict) else []
            return Evidence(
                status, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
        except Exception=)
        except Exception, rna_gtex=rna_gtex, rna_tissue&search=rna_tissue&search, tries=tries, list) else []
                genes=list) else []
                genes, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
            except Exception=)
            except Exception, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
    ...
    @router.get("/assoc/bulk-prot", response_model=response_model, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, dict=dict, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__assoc_sc(condition: str,
    limit: int = Query(100, ge=1, le=500),
) -> Evidence:
...
hybrid_assoc_sc = hybridize('assoc_sc', __core__assoc_sc, snapshot_fn=None)

@router.get("/assoc/sc", response_model=Evidence)
async def assoc_sc(condition: str,
    limit: int = Query(100, ge=1, le=500),
) -> Evidence:
    """Singleâcell signal using Human Protein Atlas search API.
    Accepts either a disease term or a gene-like token; if gene-like, normalizes and searches by gene.
    ...
    search_term = condition
    normalized_symbol: Optional[str] = None
    if _looks_like_gene_token(condition, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_assoc_sc(condition=condition, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__assoc_perturb(condition: str,
...
hybrid_assoc_perturb = hybridize('assoc_perturb', __core__assoc_perturb, snapshot_fn=None)

@router.get("/assoc/perturb", response_model=Evidence)
async def assoc_perturb(condition: str,
    symbol: Optional[str] = Query(None),
    limit: int = Query(100, ge=1, le=500),
) -> Evidence:
    """CRISPR perturbation evidence.
    ...
    if depmap_csv and symbol:
        try:
            ...
            # naive filter: keep rows mentioning symbol in first 200 chars
            for line in (csv_txt or "").splitlines()[:5000]:
                if symbol.upper() in line.upper(, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_assoc_perturb(condition=condition, symbol=symbol, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__expression_baseline(symbol: Optional[str] = Query(None),
...
hybrid_expression_baseline = hybridize('expression_baseline', __core__expression_baseline, snapshot_fn=None)

@router.get("/expr/baseline", response_model=Evidence)
async def expression_baseline(symbol: Optional[str] = Query(None),
    gene: Optional[str] = Query(None),  # accept both ?symbol= and ?gene=
    limit: int = Query(50, ge=1, le=500),
) -> Evidence:
    """Baseline expression via HPA â UniProt â Expression Atlas.
    ...
    try:
        body = await _get_json(atlas_url, tries=1)
        results: List[Dict[str, Any]] = []
        if isinstance(body, dict, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_expression_baseline(symbol=symbol, gene=gene, # accept both ?symbol=# accept both ?symbol, ) -> Evidence=) -> Evidence, CB1âCNR1).
        """
        sym_in=CB1âCNR1).
        """
        sym_in, field_name=field_name, "normalized_symbol"="normalized_symbol", "baseline"="baseline", mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__expr_localization(symbol: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
...
hybrid_expr_localization = hybridize('expr_localization', __core__expr_localization, snapshot_fn=None)

@router.get("/expr/localization", response_model=Evidence)
async def expr_localization(symbol: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
    validate_symbol(symbol, field_name="symbol")
    sym_norm = await _normalize_symbol(symbol)
    ...
    try:
        uj = await _get_json(uni, tries=1)
        locs = []
        for r in (uj.get("results", []) or [], mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_expr_localization(symbol=symbol, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, []) if isinstance(js=[]) if isinstance(js, dict) else []
            if locs=dict) else []
            if locs, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
        except Exception=)
        except Exception, tries=tries, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__expr_inducibility(symbol: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
    validate_symbol(symbol, field_name="symbol")
    sym_norm = await _normalize_symbol(symbol)
...
hybrid_expr_inducibility = hybridize('expr_inducibility', __core__expr_inducibility, snapshot_fn=None)

@router.get("/expr/inducibility", response_model=Evidence)
async def expr_inducibility(symbol: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
    validate_symbol(symbol, field_name="symbol")
    sym_norm = await _normalize_symbol(symbol)
...
async def mech_pathways(
    ...
    try:
        ...
        hits = js.get("results", []) if isinstance(js, dict) else []
        for h in hits:
            if (h.get("stId", "") or "").startswith("R-HSA", mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_expr_inducibility(symbol=symbol, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, {}).get("idlist"={}).get("idlist", [])
            return Evidence(
                status=[])
            return Evidence(
                status, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
    ...
    @router.get("/mech/pathways", response_model=response_model, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, "") or "").startswith("R-HSA"="") or "").startswith("R-HSA", mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__mech_ppi(symbol: str,
...
hybrid_mech_ppi = hybridize('mech_ppi', __core__mech_ppi, snapshot_fn=None)

@router.get("/mech/ppi", response_model=Evidence)
async def mech_ppi(symbol: str,
    species: int = Query(9606, ge=1),
    cutoff: float = Query(0.9, ge=0.0, le=1.0),
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
...
async def tract_drugs(
    ...
    try:
        ...
        tjs = await _get_json(chembl_search, tries=1)
        tids = []
        for t in (tjs.get("targets", []) if isinstance(tjs, dict) else [], mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_mech_ppi(symbol=symbol, species=species, cutoff=cutoff, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, sp=sp, tries=tries, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
            string_id=)
            string_id, tries=tries, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
        except Exception=)
        except Exception, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
    ...
    @router.get("/mech/ligrec", response_model=response_model, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, list) else []
            filtered=list) else []
            filtered, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
        except Exception=)
        except Exception, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
    ...
    @router.get("/tract/drugs", response_model=response_model, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, {}).get("target"={}).get("target", {}).get("knownDrugs"={}).get("knownDrugs", {}).get("rows"={}).get("rows", []))
            if rows=[]))
            if rows, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
...
async def __core__tract_ligandability_sm(symbol: str,
    limit: int = Query(100, ge=1, le=500),
) -> Evidence:
    validate_symbol(symbol, field_name="symbol")
    sym_norm = await _normalize_symbol(symbol)
...
hybrid_tract_ligandability_sm = hybridize('tract_ligandability_sm', __core__tract_ligandability_sm, snapshot_fn=None)

@router.get("/tract/ligandability-sm", response_model=Evidence)
async def tract_ligandability_sm(symbol: str,
    limit: int = Query(100, ge=1, le=500),
) -> Evidence:
    validate_symbol(symbol, field_name="symbol")
    sym_norm = await _normalize_symbol(symbol)
...
async def tract_ligandability_ab(
    ...
    try:
        ...
        entries: List[Any] = []
        if isinstance(js, dict, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_tract_ligandability_sm(symbol=symbol, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, []) if isinstance(js=[]) if isinstance(js, dict) else []
            return Evidence(
                status=dict) else []
            return Evidence(
                status, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )
    ...
    @router.get("/tract/ligandability-ab", response_model=response_model, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, dict=dict, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__tract_ligandability_oligo(symbol: str,
    limit: int = Query(100, ge=1, le=500),
) -> Evidence:
...
hybrid_tract_ligandability_oligo = hybridize('tract_ligandability_oligo', __core__tract_ligandability_oligo, snapshot_fn=None)

@router.get("/tract/ligandability-oligo", response_model=Evidence)
async def tract_ligandability_oligo(symbol: str,
    limit: int = Query(100, ge=1, le=500),
) -> Evidence:
    """Oligonucleotide ligandability via Ribocentre Aptamer API."""
    validate_symbol(symbol, field_name="symbol")
    ...
    try:
        ...
        for it in items:
            ligand = (it.get("Ligand") or it.get("Target") or it.get("ligand") or "")
            if isinstance(ligand, str) and sym_norm.lower() in ligand.lower(, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_tract_ligandability_oligo(symbol=symbol, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, str) and sym_norm.lower() in ligand.lower(=str) and sym_norm.lower() in ligand.lower(, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__tract_modality(symbol: str,
...
hybrid_tract_modality = hybridize('tract_modality', __core__tract_modality, snapshot_fn=None)
...
async def tract_modality(symbol: str,
    ...
    * User can override the resulting per-modality scores with w_* query params.
    ...
    async def _get_comp(, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_tract_modality(symbol=symbol, # Optional priors (rough class success priors). Override if you want.
        prior_sm=# Optional priors (rough class success priors). Override if you want.
        prior_sm, prior_ab=prior_ab, prior_oligo=prior_oligo, # Optional user overrides for the final scores; if provided=# Optional user overrides for the final scores; if provided, they replace computed scores.
        w_sm=they replace computed scores.
        w_sm, w_ab=w_ab, w_oligo=w_oligo, ) -> Evidence=) -> Evidence, field_name=field_name, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__tract_immunogenicity(symbol: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
...
hybrid_tract_immunogenicity = hybridize('tract_immunogenicity', __core__tract_immunogenicity, snapshot_fn=None)

@router.get("/tract/immunogenicity", response_model=Evidence)
async def tract_immunogenicity(symbol: str,
    limit: int = Query(50, ge=1, le=200),
) -> Evidence:
    """Immunogenicity via IEDB IQâAPI (epitope_search + tcell_search)."""
    validate_symbol(symbol, field_name="symbol")
    ...
    async def _epi(, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_tract_immunogenicity(symbol=symbol, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
...
async def __core__clin_endpoints(condition: str,
...
hybrid_clin_endpoints = hybridize('clin_endpoints', __core__clin_endpoints, snapshot_fn=None)

@router.get("/clin/endpoints", response_model=Evidence)
async def clin_endpoints(condition: str,
    limit: int = Query(3, ge=1, le=100),
) -> Evidence:
    validate_condition(condition, field_name="condition")
    base = "https://clinicaltrials.gov/api/v2/studies"
...
async def clin_rwe(
    ...
    # If symbol hints PCSK9 program, also query alirocumab/evolocumab directly
    faers_drug_urls: List[str] = []
    if (symbol or "").upper() in {"PCSK9", "LDLR", "LDLRAP1"} or "cholesterol" in condition.lower(, mode: SourceMode = Depends(get_source_mode)):
    ev = await hybrid_clin_endpoints(condition=condition, limit=limit, ) -> Evidence=) -> Evidence, field_name=field_name, tries=tries, []) if isinstance(js=[]) if isinstance(js, dict) else []
        except Exception=dict) else []
        except Exception, tries=tries, source=source, fetched_n=fetched_n, data=data, citations=citations, fetched_at=fetched_at, )

    @router.get("/clin/rwe"=)

    @router.get("/clin/rwe", response_model=response_model, symbol=symbol, limit=limit, ) -> Evidence=) -> Evidence, PCSK9), also query brand/generic mAbs in FAERS.
        """
        validate_condition(condition=PCSK9), also query brand/generic mAbs in FAERS.
        """
        validate_condition(condition, field_name=field_name, "LDLR"="LDLR", "LDLRAP1"} or "cholesterol" in condition.lower(="LDLRAP1"} or "cholesterol" in condition.lower(, mode=mode)
    payload = getattr(ev, "data", ev)
    # If payload is a dict produced by core, attempt to reconstruct Evidence; else return as-is
    try:
        from pydantic import BaseModel as _BM
