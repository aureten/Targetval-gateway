...
from app.hybrid import get_source_mode, SourceMode  # hybrid mode control
...
# -----------------------------------------------------------------------------
# Snapshots (optional placeholders) - plug real local implementations later
# -----------------------------------------------------------------------------
async def _snapshot_genetics_mr(gene: str, efo_id: str, limit: int = 50) -> "Evidence":
    return Evidence(status="NO_DATA", source="snapshot: genetics_mr", fetched_n=0,
                    data={"gene": gene, "efo_id": efo_id, "note": "snapshot placeholder"},
                    citations=[], fetched_at=_now())

async def _snapshot_genetics_sqtl(gene: str, limit: int = 50, ensembl: Optional[str] = None) -> "Evidence":
    return Evidence(status="NO_DATA", source="snapshot: genetics_sqtl", fetched_n=0,
                    data={"gene": gene, "ensembl": ensembl, "note": "snapshot placeholder"},
                    citations=[], fetched_at=_now())

async def _snapshot_expr_inducibility(symbol: str, limit: int = 50) -> "Evidence":
    return Evidence(status="NO_DATA", source="snapshot: expr_inducibility", fetched_n=0,
                    data={"symbol": symbol, "note": "snapshot placeholder"},
                    citations=[], fetched_at=_now())

async def _snapshot_tract_modality(symbol: str, prior_ab: float = 0.50, prior_sm: float = 0.50, prior_oligo: float = 0.50) -> "Evidence":
    return Evidence(status="NO_DATA", source="snapshot: tract_modality", fetched_n=0,
                    data={"symbol": symbol, "note": "snapshot placeholder"},
                    citations=[], fetched_at=_now())

async def _snapshot_clin_rwe(condition: str, symbol: Optional[str] = None, limit: int = 50) -> "Evidence":
    return Evidence(status="NO_DATA", source="snapshot: clin_rwe", fetched_n=0,
                    data={"condition": condition, "symbol": symbol, "note": "snapshot placeholder"},
                    citations=[], fetched_at=_now())

async def _snapshot_comp_freedom(symbol: str, limit: int = 100) -> "Evidence":
    return Evidence(status="NO_DATA", source="snapshot: comp_freedom", fetched_n=0,
                    data={"symbol": symbol, "note": "snapshot placeholder"},
                    citations=[], fetched_at=_now())
# -----------------------------------------------------------------------------
...
async def genetics_mr(
    ...
    efo_id: str = Query(..., alias="efo"),
    limit: int = Query(50, ge=1, le=200),
    mode: SourceMode = Depends(get_source_mode),
) -> Evidence:
    """
    ...
    # Hybrid mode gate
    if mode == "snapshot":
        return await _snapshot_genetics_mr(gene=gene, efo_id=efo_id, limit=limit)
...
async def genetics_sqtl(
    ...
    # Hybrid mode gate
    if mode == "snapshot":
        return await _snapshot_genetics_sqtl(gene=gene, limit=limit, ensembl=ensembl)
...
async def expr_inducibility(
    ...
    # Hybrid mode gate
    if mode == "snapshot":
        return await _snapshot_expr_inducibility(symbol=symbol, limit=limit)
    validate_symbol(symbol, field_name="symbol")
    sym_norm = await _normalize_symbol(symbol)
    url = (
...
async def tract_modality(
    ...
    * User can override the resulting per-modality scores with w_* query params.
    ...
    # Hybrid mode gate
    if mode == "snapshot":
        return await _snapshot_tract_modality(symbol=symbol, prior_ab=prior_ab, prior_sm=prior_sm, prior_oligo=prior_oligo)
...
async def clin_rwe(
    ...
    # Hybrid mode gate
    if mode == "snapshot":
        return await _snapshot_clin_rwe(condition=condition, symbol=symbol, limit=limit)
...
async def comp_freedom(
    ...
    # Hybrid mode gate
    if mode == "snapshot":
        return await _snapshot_comp_freedom(symbol=symbol, limit=limit)
    validate_symbol(symbol, field_name="symbol")
    sym_norm = await _normalize_symbol(symbol)
