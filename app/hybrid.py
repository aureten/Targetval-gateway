...
import time, asyncio, inspect, hashlib, json
...
def _normalize_payload(module: str, payload: Any, mode: SourceMode, source: str,
                       src_version: Optional[str], snapshot_id: Optional[str],
                       latency_ms: Optional[int], cache: Optional[str],
                       router_reason: str, fallbacks: list[dict]) -> Evidence:
    ...
    prov = Provenance(
        mode=mode, source=source, source_version=src_version,
        snapshot_id=snapshot_id, retrieved_at=_now_iso(), router_reason=router_reason
    )
    diag = Diagnostics(latency_ms=latency_ms, cache=cache, fallbacks=fallbacks)
    return Evidence(module=module, status=status, fetched_n=fetched_n, data=data, provenance=prov, diagnostics=diag)
...
class _TTLCache:
    ...
    def _ts(self) -> float: return time.time()
...
async def get_source_mode(
    ...
    return (mode or x_source_mode or "auto")  # prefer explicit query over header
...
def hybridize(
    ...
    *,
    ...
    async def _wrapped(*, mode: Optional[SourceMode] = None, **params) -> Evidence:
        chosen = (mode or _default)
        reason = "user_forced" if mode else "policy_default"

        # cache check (auto caches under resolved effective mode; we cache 'live' when live OK)
        cache_mode = chosen  # 'auto' may resolve to live/snapshot after health rules; we keep it simple here
        cached = await CACHE.get(module, cache_mode, params)
        ...
        try:
            if chosen == "snapshot":
                ...
                # auto: try live first, fallback to snapshot on error
                try:
                    payload = await _maybe_await(live_fn, **params)
                    source = source_name_live
                    effective = "live"
                    ...
                    if not snapshot_fn:
                        raise
                    fallbacks.append({"module": module, "from": "live", "to": "snapshot", "reason": str(e_live.__class__.__name__)})
                    payload = await _maybe_await(snapshot_fn, **params)
                    source = source_name_snapshot
                    effective = "snapshot"
            ...
            ev = _normalize_payload(
                module=module, payload=payload, mode=effective, source=source,
                src_version=src_version, snapshot_id=snapshot_id, latency_ms=latency_ms,
                cache="miss", router_reason=reason, fallbacks=fallbacks
            )
            await CACHE.set(module, cache_mode, params, _ttl, ev)
            return ev

        except Exception as e:
            # hard error path; surface minimal envelope
            latency_ms = int((time.time()-t0)*1000)
            ev = Evidence(
                module=module, status="UPSTREAM_ERROR", fetched_n=0, data=[],
                provenance=Provenance(mode=(mode or _default), source="ERROR", router_reason=f"exception:{e.__class__.__name__}"),
...
async def run_aggregate(
    ...
    # Simple sequential; you can switch to asyncio.gather if your callables are async
    for m in modules:
        w = wrappers.get(m)
        if not w:
            out[m] = Evidence(
                module=m, status="CLIENT_ERROR", fetched_n=0, data=[],
