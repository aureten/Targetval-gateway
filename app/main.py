...
APP_VERSION=os.getenv("APP_VERSION","2025.10-final")
...
DEFAULT_ORDER=os.getenv("AGG_DEFAULT_ORDER","sequential").lower()
DEFAULT_CONCURRENCY=int(os.getenv("AGG_CONCURRENCY","6"))
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            logging.getLogger("uvicorn.access").info(
                ...
                (time.perf_counter()-start)*1000.0, rid
            )
        except Exception:
            pass
        return response
...
def _build_key_to_path_and_domains(reg:Dict[str,Any])->Tuple[Dict[str,str],Dict[str,List[int]],List[str]]:
    ...
    for m in reg.get("modules",[]):
        ...
        keys.append(k)
        ktp[k]=p
        doms=(m.get("domains") or {}).get("primary") or []
        ktd[k]=[int(d) for d in doms if str(d).isdigit()]
    return ktp,ktd,keys
...
def create_app()->FastAPI:
    app=FastAPI(
        ...
        description="Biologic target validation API gateway with 55 live modules across 6 domains"
    )
    ...
    # Health endpoint
    @app.get("/v1/healthz", operation_id="healthCheck", tags=["system"])
    def healthz():
        """Health check endpoint"""
        return {"ok":True,"modules":len(app.state.module_keys),"version":APP_VERSION}
    
    # List modules
    @app.get("/v1/modules", operation_id="listModules", tags=["system"])
    def list_modules():
        """List all 55 available module keys"""
    ...
    async def run_callable(name:str, fn:Any, kw_all:Dict[str,Any])->Tuple[str,Dict[str,Any]]:
        ...
        try:
            ...
            if asyncio.iscoroutine(res):
                res=await res
            if hasattr(res,"dict"):
                return name, res.dict()
            if hasattr(res,"model_dump"):
                return name, res.model_dump()
    ...
    def shared_kwargs(body:Dict[str,Any])->Dict[str,Any]:
        ...
        ensg=body.get("ensembl_id") or body.get("ensembl")
        efo=body.get("efo")
    ...
    # Run single module (POST)
    @app.post("/v1/module", operation_id="runModule", tags=["modules"])
    async def run_module_post(body: ModuleRunRequest):
        """Run a single module by key with POST"""
    ...
    # Run single module (GET)
    @app.get("/v1/module/{name}", operation_id="runModuleGet", tags=["modules"])
    async def run_module_get(
        request:Request,
        name:str=Path(..., description="Module key"),
        ...
        """Run a single module by key with GET"""
        ...
        extras={k:v for k,v in request.query_params.items() if k not in known}
        _,res=await run_callable(
            name, fn,
            shared_kwargs({"gene":gene,"symbol":symbol,"ensembl":ensembl,"efo":efo,"condition":condition,"limit":limit,"extra":extras})
        )
        return res
    
    async def aggregate_impl(mod_keys:List[str], body:Dict[str,Any], order:str, cont:bool)->Dict[str,Any]:
        res:Dict[str,Any]={}
        kw_all=shared_kwargs(body)
        if order=="parallel":
            sem=asyncio.Semaphore(DEFAULT_CONCURRENCY)
            async def _g(mk:str, fn:Any):
                async with sem:
                    _,r=await run_callable(mk,fn,kw_all)
                    return mk,r
        ...
        for mk in mod_keys:
            ...
            _,r=await run_callable(mk,fn,kw_all)
            res[mk]=r
            if not cont and (not isinstance(r,dict) or r.get("status")=="ERROR"):
                break
        return res
    ...
    # Aggregate endpoint
    @app.post("/v1/aggregate", operation_id="aggregateModules", tags=["modules"])
    async def aggregate(body: AggregateRequest):
        """Run multiple modules (sequential or parallel) with optional domain filter"""
        ...
        order_map={k:i for i,k in enumerate(app.state.module_keys)}
        keys=sorted(keys, key=lambda k: order_map.get(k,1_000_000))
    ...
    # Run domain
    @app.post("/v1/domain/{domain_id}/run", operation_id="runDomain", tags=["modules"])
    async def run_domain(
        domain_id:int=Path(..., ge=1, le=6, description="Domain ID (1-6)"),
        body:Dict[str,Any]|None=Body(default=None)
    ):
        """Run all modules for a specific domain (1-6) sequentially"""
        payload=body or {}
        keys=filter_domain(
            app.state.module_keys,
            app.state.key_to_domains,
            domain_id,
