...
+Synced with router.py (includes bucket-level knowledge-graph synthesis endpoints).
+"""
+
...
+
+# Import router (must be on PYTHONPATH or same directory)
+try:
+    from router import router as targetval_router
...
+
+logger = logging.getLogger("targetval")
...
+            "/genetics/functional",
+            "/genetics/cross",
+            # ... plus all pre-existing endpoints from router.py
+        ],
+        "docs": "/docs",
+        "redoc": "/redoc",
+    }
